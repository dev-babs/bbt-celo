...
export function GameModal({ gameId, isOpen, onClose }: GameModalProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const tokenName = tokenAddress ? "TOKEN" : "ETH"; // Will be replaced with actual token name if needed
      const shareText = `Just won ${formatEther(betAmount || BigInt(0))} ${tokenName} playing BlOcXTacToe! 🎮\n\nIf you think you're good enough, come play me!\n\nGame ID: ${gameId.toString()}\nWinner: ${address.slice(0, 6)}...${address.slice(-4)}`;
